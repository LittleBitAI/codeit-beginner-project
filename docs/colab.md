...
부모 PID가 `1`인 줄이 보이면 서버는 죽었고 학습은 살아 있습니다. **그 process를 죽이지 마세요.** 그대로 두면 결과는 평소대로 S3에 올라갑니다. 죽이더라도 epoch마다 저장한 checkpoint가 남아 이어서 학습할 수 있지만, 돌고 있던 epoch은 버려집니다. 다만 그 학습의 로그는 화면에 다시 이어지지 않고 팀 활동에도 중단으로 남습니다. 같은 GPU에 새 학습을 얹지도 마세요.
...
| 세션이 끊겼다 | epoch마다 저장한 checkpoint가 `s3://<bucket>/experiments/completed/<실행 이름>/running/`에 남습니다. **새 실행 이름**으로 돌리면서 `train.resume_from`에 그 `last_checkpoint.pt` 경로를 적으면 이어서 학습합니다. `epochs`는 남은 수가 아니라 전체 목표를 그대로 적습니다 |

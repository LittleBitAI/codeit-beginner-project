...
3. **새 실험** — 0.62437을 받은 설정으로 시작합니다. 위쪽에 `점수를 받은 설정 채우기 · 최고 점수 detector` 버튼이 있으면 한 번에 채워지고, 없으면 아래 값을 손으로 넣습니다. **발표 시연이라면 `epochs`만 1이나 2로 낮추세요.** 12 epoch은 표본이 아닌 진짜 판에서 하루 가까이 걸립니다.
...
4. **평가와 제출** — 학습이 끝나면 그 실행에서 평가를 돌립니다. test manifest가 있으면 제출 CSV가 함께 만들어집니다. 화면에 `내려받기`가 있으면 그것으로 받고, 없으면 평가 결과가 알려 주는 경로(`artifacts/...` 아래)에서 파일을 가져옵니다.
5. **앙상블** — 끝난 실행 둘 이상을 골라 합칩니다. 합치기 전에 진단이 얼마나 닮았는지 알려 줍니다. 약한 실행을 넣으면 점수가 내려갑니다(7개 0.62087 < 단독 0.62437 < 상위 3개 0.62645).
6. **임베딩 학습** — 화면에서 됩니다. 다만 참조 crop을 모은 crop 은행이 먼저 있어야 하므로, 1번 준비에서 `참조 crop 은행도 만들기`를 함께 켜 두어야 합니다. 빠뜨렸다면 `이미 있으면 덮어쓰기`까지 켜고 준비를 다시 돌립니다. 5절의 재현은 **번들에 든 은행과 임베딩**을 쓰므로 이것과 무관하게 그대로 됩니다.

## 7. 알아 둘 것

- **번들의 임베딩과 crop 은행을 다시 만들지 마세요.** 참조 crop이 바뀌면 margin이 전부 바뀌어 다른 점수가 됩니다. 나중에 화면에서 임베딩을 학습하게 되더라도 그것은 기능을 보이는 것이고, 5절 재현과 섞지 않습니다.

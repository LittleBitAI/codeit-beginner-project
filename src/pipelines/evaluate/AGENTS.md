...
Takes a validation manifest plus a checkpoint or supplied predictions and produces COCO-style detection metrics with the predictions that were scored. Given an unlabeled COCO test manifest it also writes a competition submission CSV. It never trains or scores test data.

...
- `mmdetection_backend.py` reads `backend="mmdetection"` checkpoints; no `backend` key still means torchvision, and any other value stops. Contract: `contracts/proposals/012-mmdetection-checkpoint-inference.md`. Five traps:
  ...
  - mmdet 3.3.0 caps mmcv below 2.2.0, but the only extension wheel for this torch is 2.2.0 and mmdet has had no release since. `_shimmed_mmcv_version` lifts that cap for the verified range only — widening it would let a genuinely wrong pairing through and break somewhere unrecognizable instead of reporting an install problem.

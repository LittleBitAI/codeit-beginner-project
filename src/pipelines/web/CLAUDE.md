...
- `GET /api/data/datasets` lists the folders under `PROCESSED_ROOT` so a person can click a dataset instead of pasting its path — pasting was the only way, so picking between v3 and v5 meant digging through the bucket. It opens no file: one listing call, then names and which artifacts exist. The paste box stays, because a folder outside that root must still be selectable. `PROCESSED_ROOT` and the artifact file names are copied from data (no import allowed); if they drift the list is empty rather than wrong.
...
- Resuming always takes a **new** `run_id`. Reusing the old one makes train refuse to start — that run's working directory is still there — and mixes two runs into one name. `epochs` is the whole plan, not what is left, so leaving it empty carries the original target over. A run a person **stopped** resumes like a failed one: the checkpoint must exist, and the button appears only after an epoch finished. Only `interrupted` skips that check — its checkpoint may belong to the runtime that vanished.

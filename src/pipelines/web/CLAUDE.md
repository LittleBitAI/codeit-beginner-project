...
- Resuming always takes a **new** `run_id`. Reusing the old one makes train refuse to start — that run's working directory is still there — and mixes two runs into one name. `epochs` is the whole plan, not what is left, so leaving it empty carries the original target over. How it ended — stopped, failed, interrupted — does not matter; the checkpoint must be **there**. Skipping that check moves a knowable failure past creating a run and restarting the queue. The button follows the rule train saves on: `checkpoint_every` epochs, not one.

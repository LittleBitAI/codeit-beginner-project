...
- `SUMMARY_VERSION` is `"3"`, which only adds keys; every earlier key keeps its name and meaning. Summaries written earlier keep the old shape until their run is registered again, so consumers must survive `losses` or `per_class_summary` being absent, not merely `null`.

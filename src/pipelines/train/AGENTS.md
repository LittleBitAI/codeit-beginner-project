...
- Restoring a schedule has to put its learning rate back on the optimizer as well. `load_state_dict` only restores how far the schedule has come, so without that the first batch after a resume trains at the schedule's opening value and only catches up on the next update.

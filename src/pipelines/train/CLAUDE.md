...
- The MMDetection ones need `device="cuda"`, `precision="amp"`, `optimizer="AdamW"`; anything else is refused before the first batch, not partway through the night. GPU cost sits with it. `input_size` is theirs alone, refused with a torchvision architecture, not ignored.

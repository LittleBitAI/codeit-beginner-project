...
- Architectures come from the contract; never accept arbitrary builder names. Three are MMDetection, via `mmdetection_adapter.py`.
- The MMDetection trio only fits 8GB at `device="cuda"`, `precision="amp"`, `optimizer="AdamW"`, `batch_size=1`; anything else is refused before the first batch, not partway through the night. `input_size` is theirs alone, refused with a torchvision architecture, not ignored.
